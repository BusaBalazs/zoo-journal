...
import { IconArrowLeft, IconMap, IconClock, IconShield, IconBook } from '../components/icons'
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto flex flex-col bg-[var(--paper-raised)] relative overflow-hidden">
      ...
      <div className="flex-1 px-6 pt-4 pb-8 flex flex-col items-center text-center relative">
        <div className="w-20 h-20 rounded-full bg-[color:var(--green-mid)]/10 text-[var(--green-mid)] flex items-center justify-center mb-6">
          <IconMap className="w-10 h-10" />
        </div>

        {(step === STEPS.idle || step === STEPS.checking) && (
          <>
            <h1 className="font-display text-2xl text-ink mb-2">{t('startToday')}</h1>
            <p className="text-ink-soft leading-relaxed mb-7 max-w-xs">
              {t('locationBody')}
            </p>

            <div className="w-full bg-[var(--paper-raised)] border border-[var(--rule)] rounded-2xl p-4 text-left mb-7 shadow-sm">
              <p className="font-semibold text-ink mb-3">
                {ZOO.name}
                <span className="block text-xs font-normal text-ink-soft mt-0.5">{t('researchSession')}</span>
              </p>
              <div className="flex items-start gap-2.5 mb-2.5 text-sm text-ink-soft">
                <IconClock className="w-4 h-4 mt-0.5 shrink-0 text-[var(--green-mid)]" />
                {t('locationPermission')}
              </div>
              <div className="flex items-start gap-2.5 text-sm text-ink-soft">
                <IconShield className="w-4 h-4 mt-0.5 shrink-0 text-[var(--green-mid)]" />
                {t('dataSafe')}
              </div>
            </div>
            ...
            <div className="mt-8 text-xs text-ink-soft/70">
              ...
              <div className="flex items-center justify-center gap-3">
                <button onClick={() => simulate(true)} className="underline underline-offset-2 hover:text-ink-soft">
                  {t('simulateHere')}
                </button>
                <button onClick={() => simulate(false)} className="underline underline-offset-2 hover:text-ink-soft">
                  {t('simulateAway')}
                </button>
              </div>
            </div>
        ...
        {step === STEPS.matched && (
          <div className="rise-in w-full">
            <h1 className="font-display text-2xl text-ink mb-2">{t('locationConfirmed')}</h1>
        ...
        {(step === STEPS.unmatched || step === STEPS.unavailable) && (
          <div className="rise-in w-full">
            <h1 className="font-display text-2xl text-ink mb-2">{t('notAtZoo')}</h1>
            <p className="text-ink-soft leading-relaxed mb-7 max-w-xs mx-auto">
      ...
      <img
        ...
        alt=""
        className="w-full h-24 object-cover object-top pointer-events-none select-none mt-auto"
      />
    </div>
  )
}
