...
export default function Explore({ entries, active, onOpenAnimal, onCustomResearch, onRetryLocation, onGoJournal }) {
  ...
  const filtered = ANIMALS.filter((a) => a.name.toLowerCase().includes(query.toLowerCase()))

  return (
    <div className="max-w-md mx-auto px-4 py-5">
      <div className="flex items-center justify-between mb-5 gap-3">
        <h1 className="font-display text-2xl text-ink">{t('featuredAnimals')}</h1>
        {searchOpen ? (
          <input
            autoFocus
            value={query}
      ...
      <div className="space-y-2.5 mb-5">
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-ink-soft text-center py-6">{t('noAnimal')}</p>
        )}
      </div>
